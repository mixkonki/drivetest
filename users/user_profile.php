...
// Ελέγχει αν ο χρήστης είναι συνδεδεμένος, διαφορετικά ανακατευθύνει στο login
if (!isset($_SESSION['user_id'])) {
    header("Location: " . BASE_URL . "/public/login.php");
    error_log("User not logged in, redirecting to login.php from user_profile.php. BASE_URL: " . BASE_URL . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
    exit();
}
...
// Ελέγχει αν το user_id είναι σωστά ορισμένο
if (!isset($user_id) || $user_id === null) {
    error_log("User ID is not set or is null in user_profile.php for user: $username. Session data: " . print_r($_SESSION, true) . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
...
if (!$mysqli) {
    error_log("Database connection failed in user_profile.php: " . (isset($mysqli) ? $mysqli->error : 'No connection object') . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Database connection failed.']);
    exit();
}

// Ανακτά δεδομένα του χρήστη από τον πίνακα users
$query = "SELECT fullname, email, phone, address, street_number, postal_code, city, latitude, longitude, avatar 
          FROM users WHERE id = ?";
$stmt = $mysqli->prepare($query);
if (!$stmt) {
    error_log("Failed to prepare query for user_id $user_id in user_profile.php: " . $mysqli->error . " - Query: $query - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
    header('Content-Type: application/json');
...
// Ελέγχει αν βρέθηκε ο χρήστης
if (!$user) {
    error_log("User not found for ID: $user_id in user_profile.php. Query result: " . print_r($result, true) . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'User not found.']);
    exit();
}

error_log("Successfully fetched user data for ID: $user_id - Name: " . $user['fullname'] . ". BASE_URL: " . BASE_URL . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');

$is_editing = false;
// Ενεργοποιεί τη λειτουργία επεξεργασίας αν πατηθεί το κουμπί "Επεξεργασία"
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit'])) {
    $is_editing = true;
    error_log("Switching to edit mode for user_id $user_id from POST request in user_profile.php. POST data: " . print_r($_POST, true) . " - BASE_PATH: " . BASE_PATH . " - Current path: " . __DIR__, 3, BASE_PATH . '/debug_log.txt');
}
?>
...
<head>
    ...
    <!-- Φορτώνει το stylesheet για την εμφάνιση του προφίλ. Ελέγχει αν το BASE_URL είναι σωστό -->
    <link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/user.css" onerror="console.error('Failed to load user.css at ' + this.href)">
</head>
<body class="user-page" data-user-id="<?= htmlspecialchars($user_id ?? 0) ?>">
<?php require_once BASE_PATH . '/includes/header.php'; // Περιλαμβάνει το header της εφαρμογής ?>
...
<div class="container">
    ...
    <?php if (isset($_GET['success'])): ?>
        <p class="success-message" style="color: #4CAF50; margin-top: 10px; background-color: #e8f5e9; padding: 10px; border-radius: 5px; margin-bottom: 15px;"><?= htmlspecialchars($_GET['success']) ?></p>
    <?php endif; ?>
    <?php if (isset($_GET['error'])): ?>
        <p class="error-message" style="color: #aa3636; margin-top: 10px; background-color: #ffebee; padding: 10px; border-radius: 5px; margin-bottom: 15px;"><?= htmlspecialchars($_GET['error']) ?></p>
    <?php endif; ?>
</div>

<?php require_once BASE_PATH . '/includes/footer.php'; // Περιλαμβάνει το footer της εφαρμογής ?>

<script src="https://code.jquery.com/jquery-3.6.0.min.js" onerror="console.error('Failed to load jQuery')"></script>
<!-- Φορτώνει τη βιβλιοθήκη Google Maps με το API key από το config.php -->
<script src="https://maps.googleapis.com/maps/api/js?key=<?= htmlspecialchars($config['google_maps_api_key']) ?>&libraries=places&callback=initMap" async defer onerror="console.error('Failed to load Google Maps API')"></script>
<script>
    // Ορίζει το userId και το base URL για το JavaScript. Ελέγχει αν το BASE_URL είναι σωστό
    const userId = <?= htmlspecialchars($user_id) ?>;
    const drivetestConfig = { baseUrl: "<?= htmlspecialchars(BASE_URL) ?>" };
    console.log('User ID and Base URL initialized - userId:', userId, 'Base URL:', drivetestConfig.baseUrl, 'BASE_PATH:', '<?= BASE_PATH ?>');
</script>
<!-- Φορτώνει το JavaScript για τη διαχείριση του προφίλ -->
<script src="<?= BASE_URL ?>/assets/js/user_profile.js" onerror="console.error('Failed to load user_profile.js at ' + this.src)"></script>
</body>
</html>